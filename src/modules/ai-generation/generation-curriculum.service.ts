...
export class GenerationCurriculumService {
  ...
  async resolveAllTopics(dto: CreateGenerationDto): Promise<CreateGenerationDto> {
    const resolved = [];
    for (const unit of dto.units) {
      if (!unit.allTopics) {
        if (!unit.topicId)
          throw new BadRequestException('topicId is required unless allTopics is true');
        resolved.push({ ...unit, topicId: unit.topicId, allTopics: false });
        continue;
      }
      if (unit.topicId)
        throw new BadRequestException('topicId must be omitted when allTopics is true');
      const rows = await this.data.query(
        `SELECT id FROM topics WHERE chapter_id=$1 AND status=$2 ORDER BY "order" ASC,id ASC`,
        [unit.chapterId, CurriculumStatus.ACTIVE],
      );
      if (!rows.length) throw new BadRequestException('Selected Chapter has no active Topics');
      for (const row of rows as Array<{ id: string }>)
        resolved.push({ ...unit, topicId: row.id, allTopics: false });
    }
    return { ...dto, units: resolved };
  }
