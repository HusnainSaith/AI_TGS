export const configuration = () => ({
  ...
  queue: {
    ...
    prefix: process.env.QUEUE_PREFIX ?? 'tgs',
    ...
    reconciliationIntervalMs: Number(process.env.QUEUE_RECONCILIATION_INTERVAL_MS ?? 30000),
    ingestionConcurrency: Number(process.env.WORKER_INGESTION_CONCURRENCY ?? 2),
    embeddingConcurrency: Number(process.env.WORKER_EMBEDDING_CONCURRENCY ?? 2),
    aiGenerationConcurrency: Number(process.env.WORKER_AI_GENERATION_CONCURRENCY ?? 1),
    pdfConcurrency: Number(process.env.WORKER_PDF_CONCURRENCY ?? 2),
  },
  storage: {
