...
async function main() {
  ...
  try {
    ...
    const before = await db.query(`SELECT COALESCE(SUM(used),0)::int AS used FROM usage_counters WHERE subscription_id=$1`, [id.subscription]);
    ...
    const after = await db.query(`SELECT COALESCE(SUM(used),0)::int AS used FROM usage_counters WHERE subscription_id=$1`, [id.subscription]);
    result.generation = { status: generationResult.status, requestedCount: generationResult.requestedCount, generatedCount: generationResult.generatedCount, failedCount: generationResult.failedCount, questions: generatedQuestions.length, citationCount: citations[0]?.count ?? 0, quotaDelta: Number(after[0]?.used ?? 0)-Number(before[0]?.used ?? 0) };
    ...
    const failAfterRows = await db.query(`SELECT COALESCE(SUM(used),0)::int AS used FROM usage_counters WHERE subscription_id=$1`, [id.subscription]);
    result.insufficientGeneration = { status: failedResult.status, generatedCount: failedResult.generatedCount, quotaDelta: Number(failAfterRows[0]?.used ?? 0)-failBefore };
